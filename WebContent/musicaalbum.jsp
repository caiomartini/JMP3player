...
<body>
	...
	<div id="content" align="center">
		<h1><i class="fa fa-search"></i> Pesquisar Músicas por Album</h1>
		<table class="table">
			<tr>
				<td></td>
				<td><p>ID</p></td>
			...
			for(Album album : MP3Player.repositorio.ListaAlbums) {
				if(album == null) {
					...
					%>
					<tr>
						<td><p><i class="fa fa-folder-open-o"></i></p></td>
		...
		<form method="get">			
			<p>Nome do Album: </p><input type="text" name="album" value="<%=request.getParameter("album") != null ? request.getParameter("album") : ""%>">
			...
			try 
			{
				if(request.getParameter("album") != null) {
					...
					String nomeAlb = request.getParameter("album");
					for(Album album : MP3Player.repositorio.ListaAlbums) {
						if(album == null) {
							break;
						} else {
							if(nomeAlb.toUpperCase().equals(album.getAlbum().toUpperCase())) {
								musicasAlbum = MP3Player.repositorio.localizarMusica(album);
							}
						}
					}
					if(musicasAlbum.size() > 0) {
						%>
