...
<body>
	...
	<div id="content" align="center">
		<h1><i class="fa fa-search"></i> Pesquisar Músicas por Artista</h1>
		<table class="table">
			<tr>
				<td></td>
				<td><p>ID</p></td>
		...
		<form method="get">			
			<p>Nome do Artista: </p><input type="text" name="nome" value="<%=request.getParameter("nome") != null ? request.getParameter("nome") : ""%>">
			<button class="btn" type="submit">Pesquisar <i class="fa fa-search"></i></button>		
			<%
			try 
			{
				if(request.getParameter("nome") != null) {
					ArrayList<Musica> musicasArtista = new ArrayList<Musica>();
					String nomeArt = request.getParameter("nome");
					for(Artista artista : MP3Player.repositorio.ListaArtistas) {
						if(artista == null) {
							break;
						} else {
							if(nomeArt.toUpperCase().equals(artista.getNome().toUpperCase())) {
								musicasArtista = MP3Player.repositorio.localizarMusica(artista);
							}
						}
					}
					if(musicasArtista.size() > 0) {
						%>
